...
/// ABI version for dynamic plugin loader entrypoints.
///
/// This is checked by the core before invoking a plugin create symbol.
pub const PLUGIN_LOADER_ABI_VERSION: u32 = 1;

/// Opaque pointer used across the C ABI boundary for dynamic plugin loading.
pub type PluginOpaqueHandle = *mut c_void;
...
/// exports:
/// - `janus_plugin_abi_version() -> u32` (or plugin-specific fallback symbol)
/// - `janus_plugin_create() -> PluginOpaqueHandle` (or a plugin-specific
///   fallback create symbol)
#[async_trait]
pub trait JanusPlugin: Send + Sync + 'static {
    ...
    async fn init(&mut self, callbacks: Arc<dyn PluginCallbacks>, config_path: &Path)
        -> Result<()>;
    ...
    async fn handle_admin_message(&self, _body: serde_json::Value) -> Result<serde_json::Value> {
...
/// Convert a concrete plugin into an opaque FFI handle.
///
/// The extra `Box` ensures we pass a thin pointer across `extern "C"`.
pub fn into_ffi_plugin<T>(plugin: T) -> PluginOpaqueHandle
where
    T: JanusPlugin,
{
    let plugin: Box<dyn JanusPlugin> = Box::new(plugin);
    Box::into_raw(Box::new(plugin)) as PluginOpaqueHandle
}

/// Reconstruct a plugin from an opaque FFI handle.
///
/// # Safety
/// `raw` must have been created by [`into_ffi_plugin`] in the same process.
pub unsafe fn from_ffi_plugin(raw: PluginOpaqueHandle) -> Box<dyn JanusPlugin> {
    assert!(!raw.is_null(), "null plugin handle");
    *unsafe { Box::from_raw(raw as *mut Box<dyn JanusPlugin>) }
}
