...
pub async fn whip_offer(
    ...
    let offer = match SdpOffer::from_sdp_string(&body) {
        Ok(o) => o,
        Err(e) => {
            return error_response(StatusCode::BAD_REQUEST, &format!("Invalid SDP offer: {e}"));
        }
    };
...
pub async fn patch_resource(
    ...
    if if_match == "*" {
        ...
        let remote_pwd = match frag.ice_pwd {
            Some(p) => p,
            None => {
                return error_response(StatusCode::BAD_REQUEST, "Missing ice-pwd in SDP fragment");
            }
        };
...
pub async fn delete_resource(
    ...
    match resource.kind {
        ResourceKind::Whip => {
            ...
            // Close all subscriber PeerConnections (WHEP and WhipOut)
            for sub_id in subscriber_ids {
                if let Some((_, sub_resource)) = state.resources.remove(&sub_id) {
                    sub_resource.pc_handle.close().await;
                    // For WhipOut resources, also DELETE the remote resource
                    if sub_resource.kind == ResourceKind::WhipOut {
                        if let Some(url) = &sub_resource.remote_resource_url {
                            let _ = crate::whip_client::whip_out_delete(
                                url,
                                sub_resource.remote_bearer_token.as_deref(),
                            )
                            .await;
                        }
                        info!(subscriber = %sub_id, "closed WHIP-out relay (publisher removed)");
                    } else {
        ...
        ResourceKind::WhipOut => {
            // Unregister from publisher's subscriber list
            if let Some(pub_id) = resource.publisher_id {
                state.fanout.remove_subscriber(pub_id, resource_id);
            }
            // DELETE the remote resource
            if let Some(url) = &resource.remote_resource_url {
                let _ = crate::whip_client::whip_out_delete(
                    url,
                    resource.remote_bearer_token.as_deref(),
                )
                .await;
            }
            info!(resource = %resource_id, "WHIP-out relay deleted");
        }
    }
...
// ---------------------------------------------------------------------------
// POST /whip-out/:publisher_id — Add a WHIP-out relay target
// ---------------------------------------------------------------------------

/// Request body for creating a WHIP-out relay.
#[derive(serde::Deserialize)]
pub struct WhipOutRequest {
    /// Remote WHIP endpoint URL.
    pub endpoint: String,
    /// Optional Bearer token for authentication with the remote endpoint.
    pub bearer_token: Option<String>,
}

/// Response body for a created WHIP-out relay.
#[derive(serde::Serialize)]
pub struct WhipOutInfo {
    pub id: String,
    pub endpoint: String,
    pub remote_resource_url: Option<String>,
}

pub async fn whip_out_add(
    State(state): State<WhipWhepState>,
    Path(publisher_id_str): Path<String>,
    axum::Json(body): axum::Json<WhipOutRequest>,
) -> Response {
    // Parse publisher ID
    let publisher_id: ResourceId = match publisher_id_str.parse() {
        Ok(id) => id,
    ...
    let resource_id = ResourceId::new();
    let config = crate::whip_client::WhipOutConfig {
        endpoint_url: body.endpoint.clone(),
        bearer_token: body.bearer_token.clone(),
    };

    // Negotiate with remote WHIP endpoint
    let (pc_handle, remote_resource_url) =
        match crate::whip_client::whip_out_publish(&config, state.ice_lite, resource_id).await {
            Ok(result) => result,
            Err(e) => {
                return error_response(
                    StatusCode::BAD_GATEWAY,
                    &format!("WHIP-out negotiation failed: {e}"),
                );
            }
        };

    // Register as subscriber in fan-out (same as WHEP)
    state
        .fanout
        .add_subscriber(publisher_id, resource_id, pc_handle.clone());
    ...
    let resource = Resource {
        id: resource_id,
        kind: ResourceKind::WhipOut,
        etag,
        pc_handle,
        created_at: Instant::now(),
        publisher_id: Some(publisher_id),
        remote_resource_url: remote_resource_url.clone(),
        remote_bearer_token: body.bearer_token,
        remote_endpoint: Some(body.endpoint.clone()),
    };
    state.resources.insert(resource_id, resource);
    ...
    info!(
        ...
        "WHIP-out relay created"
    );

    let response_body = WhipOutInfo {
...
// ---------------------------------------------------------------------------
// GET /whip-out/:publisher_id — List WHIP-out relay targets
// ---------------------------------------------------------------------------

pub async fn whip_out_list(
    State(state): State<WhipWhepState>,
    Path(publisher_id_str): Path<String>,
) -> Response {
    // Parse publisher ID
    ...
    // Collect all WhipOut resources for this publisher
    let relays: Vec<WhipOutInfo> = state
        .resources
        .iter()
        .filter(|r| r.kind == ResourceKind::WhipOut && r.publisher_id == Some(publisher_id))
        .map(|r| WhipOutInfo {
            id: r.id.to_string(),
            endpoint: r.remote_endpoint.clone().unwrap_or_default(),
            remote_resource_url: r.remote_resource_url.clone(),
        })
        .collect();
    ...
    (
        StatusCode::OK,
        headers,
        serde_json::to_string(&relays).unwrap(),
    )
        .into_response()
}
