//! Janus AudioBridge plugin — audio mixing bridge (stub).
...
#[async_trait]
impl janus_plugin_api::JanusPlugin for AudioBridgePlugin {
    ...
    fn description(&self) -> &'static str {
        "Audio mixing bridge for multi-party audio conferencing."
    }
    ...
    async fn init(
        ...
        info!("AudioBridge plugin initialized");
        self.callbacks = Some(callbacks);
        Ok(())
    }

    async fn destroy(&mut self) -> janus_plugin_api::Result<()> {
        info!("AudioBridge plugin destroyed");
        self.callbacks = None;
        Ok(())
    }
...
#[no_mangle]
pub extern "C" fn janus_plugin_audiobridge_abi_version() -> u32 {
    janus_plugin_api::PLUGIN_LOADER_ABI_VERSION
}

/// FFI entry point for dynamic loading.
#[no_mangle]
pub extern "C" fn janus_plugin_audiobridge_create() -> janus_plugin_api::PluginOpaqueHandle {
    janus_plugin_api::into_ffi_plugin(AudioBridgePlugin::default())
}
...
mod tests {
    ...
    #[test]
    fn plugin_metadata() {
        let plugin = AudioBridgePlugin::default();
        assert_eq!(plugin.name(), "Janus AudioBridge plugin");
        assert_eq!(plugin.package(), "janus.plugin.audiobridge");
    ...
    fn ffi_create_returns_valid_plugin() {
        let raw = janus_plugin_audiobridge_create();
        assert!(!raw.is_null());
        unsafe {
