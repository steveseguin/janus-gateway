...
impl VideoRoomPlugin {
    ...
    fn handle_join_subscriber(
        ...
        debug!(user_id = user_id, room = room_id, feed = feed, "subscriber joined");
                ...
                if let Some(pub_session) = self.publisher_handles.get(pub_entry.key()) {
                    if let Some(ref callbacks) = self.callbacks {
                        ...
                        tokio::spawn(async move {
                            let _ = cb
                                .push_event(&session, "", event_clone, None)
                                .await;
                        });
                    }
                }
...
impl janus_plugin_api::JanusPlugin for VideoRoomPlugin {
    ...
    async fn handle_message(
        ...
        let msg: VideoRoomRequest =
            serde_json::from_value(body).map_err(|e| janus_plugin_api::Error::Plugin(e.to_string()))?;
        ...
        match msg.request.as_str() {
            ...
            "destroy" => {
                let room_id = msg.room.ok_or_else(|| {
                    janus_plugin_api::Error::Plugin("Missing room ID".into())
                })?;
                self.rooms
                    .destroy(room_id)
                    .ok_or_else(|| janus_plugin_api::Error::Plugin(format!("No such room {room_id}")))?;
                Ok(PluginResult::Ok(PluginResultPayload {
            ...
            "listparticipants" => {
                ...
                let result = self
                    ...
                    .map_err(|e| janus_plugin_api::Error::Plugin(e))?;
                Ok(PluginResult::Ok(PluginResultPayload {
                    body: result,
                    jsep: None,
                }))
                ...
                for handle_entry in self.handles.iter() {
                    ...
                    } = handle_entry.value()
                    {
                        if *sub_uid == *sub.key() && *active {
                            let sub_session = PluginSession::new(
                                handle_entry.key().0,
                                handle_entry.key().1,
                            );
                            if let Some(ref callbacks) = self.callbacks {
...
#[no_mangle]
pub extern "C" fn janus_plugin_videoroom_create() -> *mut dyn janus_plugin_api::JanusPlugin {
...
mod tests {
    ...
    async fn create_and_destroy_room() {
        ...
        // Destroy the room
        let result = plugin
            .handle_message(&session, "t2", json!({"request": "destroy", "room": 5678}), None)
            .await
            .unwrap();
        match result {
            PluginResult::Ok(payload) => {
    ...
    async fn check_room_exists() {
        ...
        let result = plugin
            .handle_message(&session, "t1", json!({"request": "exists", "room": 1234}), None)
            .await
            .unwrap();
        ...
        let result = plugin
            .handle_message(&session, "t2", json!({"request": "exists", "room": 9999}), None)
            .await
            .unwrap();
