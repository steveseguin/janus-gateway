//! Janus TextRoom plugin — DataChannel-based text rooms (stub).
...
/// The TextRoom plugin (stub).
#[derive(Default)]
pub struct TextRoomPlugin {
    callbacks: Option<Arc<dyn PluginCallbacks>>,
}

#[async_trait]
impl janus_plugin_api::JanusPlugin for TextRoomPlugin {
    ...
    fn description(&self) -> &'static str {
        "DataChannel-based text rooms for messaging."
    }
    ...
    async fn init(
        ...
        info!("TextRoom plugin initialized");
        self.callbacks = Some(callbacks);
        Ok(())
    }

    async fn destroy(&mut self) -> janus_plugin_api::Result<()> {
        info!("TextRoom plugin destroyed");
        self.callbacks = None;
        Ok(())
    }
...
#[no_mangle]
pub extern "C" fn janus_plugin_textroom_abi_version() -> u32 {
    janus_plugin_api::PLUGIN_LOADER_ABI_VERSION
}

/// FFI entry point for dynamic loading.
#[no_mangle]
pub extern "C" fn janus_plugin_textroom_create() -> janus_plugin_api::PluginOpaqueHandle {
    janus_plugin_api::into_ffi_plugin(TextRoomPlugin::default())
}
...
mod tests {
    ...
    #[test]
    fn plugin_metadata() {
        let plugin = TextRoomPlugin::default();
        assert_eq!(plugin.name(), "Janus TextRoom plugin");
        assert_eq!(plugin.package(), "janus.plugin.textroom");
    ...
    fn ffi_create_returns_valid_plugin() {
        let raw = janus_plugin_textroom_create();
        assert!(!raw.is_null());
        unsafe {
