...
impl janus_plugin_api::JanusPlugin for EchoTestPlugin {
    ...
    async fn handle_message(
        ...
        let msg: EchoTestMessage = serde_json::from_value(body.clone()).unwrap_or(EchoTestMessage {
            audio: true,
            video: true,
            bitrate: 0,
        });
        ...
        if let Some(mut s) = self.sessions.get_mut(&key) {
            ...
            return Err(janus_plugin_api::Error::SessionNotFound(session.to_string()));
        }
...
#[no_mangle]
pub extern "C" fn janus_plugin_echotest_create() -> *mut dyn janus_plugin_api::JanusPlugin {
