...
impl janus_plugin_api::JanusPlugin for StreamingPlugin {
    ...
    async fn destroy_session(&self, session: &PluginSession) -> janus_plugin_api::Result<()> {
        ...
        if let Some((_, HandleState::Viewer { mountpoint_id, .. })) =
            self.handle_states.remove(&key)
        {
    ...
    async fn handle_message(
        ...
        match request {
            ...
            "info" => {
                ...
                let mp = self.mountpoints.get(id).ok_or_else(|| {
            ...
            "start" => {
                let mut state = self
                    .handle_states
                    .get_mut(&key)
                    .ok_or_else(|| janus_plugin_api::Error::SessionNotFound(session.to_string()))?;
...
#[no_mangle]
pub extern "C" fn janus_plugin_streaming_abi_version() -> u32 {
    janus_plugin_api::PLUGIN_LOADER_ABI_VERSION
}

/// FFI entry point for dynamic loading.
#[no_mangle]
pub extern "C" fn janus_plugin_streaming_create() -> janus_plugin_api::PluginOpaqueHandle {
    janus_plugin_api::into_ffi_plugin(StreamingPlugin::default())
}
...
mod tests {
    ...
    async fn mountpoint_info() {
        ...
        let result = plugin
            .handle_message(&session, "txn1", json!({"request": "info", "id": 1}), None)
            .await
            .unwrap();
    ...
    async fn pause_viewer() {
        ...
        let result = plugin
            .handle_message(&session, "txn2", json!({"request": "pause"}), None)
            .await
            .unwrap();
    ...
    async fn stop_viewer() {
        ...
        let result = plugin
            .handle_message(&session, "txn2", json!({"request": "stop"}), None)
            .await
            .unwrap();
    ...
    async fn unknown_request_errors() {
        ...
        let result = plugin
            .handle_message(&session, "txn1", json!({"request": "invalid"}), None)
            .await;
