...
use janus_plugin_api::{
    HandleId, JanusPlugin, Jsep, JsepType, PluginCallbacks, PluginResult, PluginSession,
...
impl JanusServer {
    ...
    pub fn transport_callbacks(
        self: &Arc<Self>,
    ) -> Arc<dyn janus_transport_api::TransportCallbacks> {
    ...
    pub async fn process_request(
        ...
        match janus {
            ...
            "detach" => {
                ...
                self.handle_detach(&transaction, SessionId(session_id), HandleId(handle_id))
                    .await
            }
            "keepalive" => {
                let session_id = message["session_id"].as_u64().unwrap_or(0);
    ...
    async fn handle_destroy(&self, transaction: &str, session_id: SessionId) -> serde_json::Value {
    ...
    async fn handle_attach(
        ...
        // Validate plugin before creating a handle so we don't create orphan handles.
        let plugin_ref = match self.plugins.get(plugin) {
            Some(plugin_ref) => Arc::clone(plugin_ref.value()),
            None => {
                return json!({
                    "janus": "error",
                    "transaction": transaction,
                    "error": {
                        "code": 460,
                        "reason": format!("Plugin '{}' not found", plugin)
                    }
                });
            }
        };

        match self.sessions.attach_handle(session_id, plugin.to_string()) {
            Ok(handle_id) => {
                // Notify plugin of new session.
                let ps = PluginSession::new(session_id, handle_id);
                if let Err(e) = plugin_ref.create_session(&ps).await {
                    // Roll back the handle so the session map and plugin state stay consistent.
                    let _ = self.sessions.detach_handle(session_id, handle_id);
                    warn!(error = %e, plugin = plugin, "plugin create_session failed");
                    return json!({
                        "janus": "error",
                        "transaction": transaction,
                        "error": {
                ...
                if let Some(jsep_resp) = payload.jsep {
                    if jsep_resp.jsep_type == JsepType::Answer {
                        ...
                        if let Some(ref offer_jsep) = jsep {
                            match self
                                .setup_peer_connection(session_id, handle_id, &ps, offer_jsep)
                                .await
                            {
                                Ok(answer_sdp) => {
                                    event["jsep"] = json!({
                            ...
                            event["jsep"] = serde_json::to_value(&jsep_resp).unwrap_or_default();
                        }
                    } else {
                        event["jsep"] = serde_json::to_value(&jsep_resp).unwrap_or_default();
                    }
        ...
        if let Some(pc) = self.peer_connections.get(&key) {
            if let Some(candidate_obj) = message.get("candidate") {
                let candidate = candidate_obj["candidate"]
                    ...
                    .unwrap_or("")
                    .to_string();
                let mid = candidate_obj["sdpMid"].as_str().unwrap_or("0").to_string();
...
impl PluginCallbacks for CorePluginCallbacks {
    fn relay_rtp(&self, session: &PluginSession, packet: &RtpPacket) {
        ...
        let _ = self
            .server
            .relay_sender
            .try_send(relay::RelayCommand::OutgoingRtp {
    ...
    fn relay_rtcp(&self, session: &PluginSession, packet: &RtcpPacket) {
        ...
        let _ = self
            .server
            .relay_sender
            .try_send(relay::RelayCommand::OutgoingRtcp {
                session_id: session.session_id,
                handle_id: session.handle_id,
                packet: packet.clone(),
            });
...
mod tests {
    ...
    async fn handle_ping() {
        ...
        let resp = server
            .process_request(&req, json!({"janus": "ping", "transaction": "txn1"}))
            .await;
    ...
    async fn handle_create_and_destroy() {
        ...
        // Create session
        let resp = server
            .process_request(&req, json!({"janus": "create", "transaction": "txn3"}))
            .await;
    ...
    async fn unknown_request_returns_error() {
        ...
        let req = TransportRequest::new("test-client");
        let resp = server
            .process_request(&req, json!({"janus": "blah", "transaction": "t1"}))
            .await;
        assert_eq!(resp["janus"], "error");
    ...
    #[tokio::test]
    async fn attach_unknown_plugin_errors() {
        let server = test_server();
        let req = TransportRequest::new("test-client");
        ...
        let resp = server
            .process_request(
                ...
                json!({
                    ...
                    "transaction": "t2",
                    "session_id": session_id,
                    "plugin": "janus.plugin.doesnotexist"
                }),
            )
            .await;
        assert_eq!(resp["janus"], "error");
        assert_eq!(resp["error"]["code"], 460);
    }
    ...
    async fn missing_transaction_still_works() {
        let server = test_server();
        let req = TransportRequest::new("test-client");
        let resp = server.process_request(&req, json!({"janus": "ping"})).await;
        assert_eq!(resp["janus"], "pong");
