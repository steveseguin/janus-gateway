...
mod tests {
    ...
    fn parse_minimal_config() {
        let toml = r#"
            ...
            session_timeout = 30
        "#;
        let cfg = JanusConfig::from_str(toml).unwrap();
    ...
    fn parse_empty_string_yields_defaults() {
        let cfg = JanusConfig::from_str("").unwrap();
        assert_eq!(cfg.general.session_timeout, 60);
    }

    #[test]
    fn parse_invalid_toml_returns_error() {
        let result = JanusConfig::from_str("{{{{");
    ...
    fn config_roundtrip_via_toml() {
        ...
        let parsed = JanusConfig::from_str(&serialized).unwrap();
    ...
    fn unknown_fields_are_ignored() {
        ...
        let result = JanusConfig::from_str(toml);
    ...
    fn parse_multiple_disabled_plugins() {
        ...
        assert!(cfg.plugins.disable.contains(&"janus.plugin.lua".to_string()));
    }
