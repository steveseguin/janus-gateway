...
impl SessionManager {
    ...
    pub fn destroy_session(&self, id: SessionId) -> crate::Result<Vec<HandleInfo>> {
        ...
        let handles: Vec<HandleInfo> = session.handles.into_iter().map(|(_, info)| info).collect();
...
mod tests {
    ...
    fn destroy_session_returns_handles() {
        ...
        let h1 = mgr
            .attach_handle(sid, "janus.plugin.echotest".into())
            .unwrap();
        let h2 = mgr
            .attach_handle(sid, "janus.plugin.videoroom".into())
            .unwrap();
