...
/// Build a test app returning both the router and the state (for assertions).
fn test_app_with_state() -> (axum::Router, WhipWhepState) {
    let server = Arc::new(JanusServer::new(JanusConfig::default()));
    let janus_router = janus_api_router(Arc::clone(&server));

    let nat = NatConfig::default();
    let whip_state = WhipWhepState::new(&nat);
    let whip_router = janus_whip_whep::whip_whep_router(whip_state.clone());

    (janus_router.merge(whip_router), whip_state)
}
...
async fn options_whip_returns_cors_and_accept_patch() {
    ...
    assert_eq!(
        resp.headers().get("accept-patch").unwrap().to_str().unwrap(),
...
async fn options_whip_includes_link_headers_when_stun_configured() {
    ...
    assert!(link.is_some(), "Link header should be present with STUN server");
