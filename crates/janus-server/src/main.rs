...
async fn main() {
    ...
    if let Err(e) = echotest
        .init(plugin_callbacks, &PathBuf::from("/etc/janus"))
        .await
    {
    ...
    let static_dir = std::env::var("JANUS_STATIC_DIR")
    ...
    if let Err(e) = start_http_transport(&http_config, Arc::clone(&server), &server.config().nat).await {
